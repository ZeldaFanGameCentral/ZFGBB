...
public interface UserDeletionMapper {
	...
	@Update("""
			...
			set sender_user_id = null, sender_name = '[deleted]', migration_hash = null, updated_ts = current_timestamp
	...
	@Update("""
			<script>
			update zfgbb.personal_message_conversation set migration_hash = null
			where personal_message_conversation_id in
			  <foreach item='conversationId' collection='conversationIds' open='(' separator=',' close=')'>#{conversationId}</foreach>
			</script>
			""")
	int scrubConversationMigrationHashes(@Param("conversationIds") List<Integer> conversationIds);

	@Update("""
			<script>
			update zfgbb.personal_message set migration_hash = null
			where migration_hash is not null and personal_message_conversation_id in
			  <foreach item='conversationId' collection='conversationIds' open='(' separator=',' close=')'>#{conversationId}</foreach>
			</script>
			""")
	int scrubPersonalMessageMigrationHashesInConversations(@Param("conversationIds") List<Integer> conversationIds);
	...
	@Update("update zfgbb.user_warning set migration_hash = null where user_id = #{userId}")
	int scrubReceivedWarningMigrationHashes(@Param("userId") Integer userId);

	@Update("""
			update zfgbb.moderation_log set actor_user_id = null, migration_hash = null
	...
	@Update("""
			...
			set target_user_id = null, target_name = '[deleted]', migration_hash = null
	...
	@Update("""
			update zfgbb.moderation_log
			set target_name = '[deleted]', migration_hash = null
