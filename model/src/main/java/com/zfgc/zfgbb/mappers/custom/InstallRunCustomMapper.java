...
public interface InstallRunCustomMapper {
	...
	@Update("""
			update zfgbb.install_run
			 ...
			 where install_id=1
			""")
	int claimInstall(@Param("fingerprint") String fingerprint,
	...
	@Insert("""
			insert into zfgbb.install_run (install_id, state, last_completed_state, request_version,
			       request_fingerprint, provision_recycle_bin, site_name, install_strategy)
			values (1, 'READY', 'READY', 1, #{fingerprint}, #{recycle}, #{siteName}, #{strategy})
