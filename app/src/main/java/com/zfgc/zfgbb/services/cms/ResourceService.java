...
public class ResourceService {
	...
	public PagedResult<Resource> getResources(String search, String type, String author, Boolean hasDownload,
		...
		return resourceDataProvider.getResources(CatalogSupport.blankToNull(search), CatalogSupport.blankToNull(type),
				CatalogSupport.blankToNull(author), hasDownload, sort, CatalogSupport.clampPage(page),
				CatalogSupport.clampPageSize(pageSize));
	}
	...
	public Resource getResource(String slug) {
		...
		contentRenderer.renderPage(resource.getPage(), ContentScope.RESOURCE);
		return resource;
	}
	...
	public ResourceShowcase getResourceShowcase() {
		...
		List<Resource> recent = rail(resources, "newest", CatalogSupport.SHOWCASE_RECENT, excludeSlug);
		List<Resource> random = rail(resources, "random", CatalogSupport.SHOWCASE_RAIL, excludeSlug);
		List<Resource> topRated = rail(resources, "rating", CatalogSupport.SHOWCASE_RAIL, excludeSlug);
		List<Resource> mostDownloaded = rail(resources, "downloads", CatalogSupport.SHOWCASE_RAIL, excludeSlug);
	...
	public Resource startResourceDiscussion(String slug, User user) {
		...
		Integer threadId = forumService.createDiscussionThread(resource.getTitle(),
				"Discussion thread for the resource [b]" + resource.getTitle() + "[/b].", user);
