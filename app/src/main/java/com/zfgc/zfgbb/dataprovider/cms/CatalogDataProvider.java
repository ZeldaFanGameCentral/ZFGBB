...
public class CatalogDataProvider {
	...
	Map<Integer, String> displayNames(Stream<Integer> userIds) {
		...
		UserDboExample usersByIdExample = new UserDboExample();
		usersByIdExample.createCriteria().andUserIdIn(ids);
		userDao.get(usersByIdExample).forEach(user -> names.put(user.getUserId(), user.getDisplayName()));
		return names;
	}
	...
	public List<ReleasedResource> deleteContentResourcesIfUnreferenced(List<Integer> candidateResourceIds) {
		if (candidateResourceIds.isEmpty())
			return List.of();
		List<Integer> deletableResourceIds = contentResourceDao.findUnreferencedContentResourceIds(candidateResourceIds);
	...
	public List<ReleasedResource> deleteContentResourceRows(List<Integer> resourceIds) {
		if (resourceIds.isEmpty())
			return List.of();
		ContentResourceDboExample resourcesExample = new ContentResourceDboExample();
		resourcesExample.createCriteria().andContentResourceIdIn(resourceIds);
		List<ReleasedResource> released = contentResourceDao.get(resourcesExample).stream()
				.map(resource -> new ReleasedResource(resource.getContentResourceId(), resource.getStorageDir(),
						resource.getFilename()))
				.toList();
		contentResourceDao.deleteWhere(resourcesExample);
		return released;
	}
}
