...
public final class BBCodeText implements BBCodeNode {
	...
	public static String withoutItsLeadingBlankRun(String text) {
		return LEADING_BLANK_RUN.matcher(text).replaceFirst("");
	}

	public static String withoutItsTrailingBlankRun(String text) {
		return TRAILING_BLANK_RUN.matcher(text).replaceFirst("");
	}

	public static boolean everyCharacterIsBlankOnceItsLineBreaksAreRead(String text) {
		return withoutItsLeadingBlankRun(text).isEmpty();
	}

	public static String authoredLineBreaksAsNewlines(String sourceText) {
