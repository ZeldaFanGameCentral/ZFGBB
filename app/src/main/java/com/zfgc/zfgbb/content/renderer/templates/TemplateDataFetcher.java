...
public class TemplateDataFetcher {
	...
	private Object endpointFetchedWithinTheConnectionBudget(Endpoint endpoint, Map<String, String> params)
	...
	public Object fetch(String path) {
		...
		try {
			...
			Optional<Endpoint> endpoint = resolve(pathPart, params);
			if (endpoint.isEmpty()) {
				return null;
			}
			Object result = endpointFetchedWithinTheConnectionBudget(endpoint.get(), params);
			return result == null ? null : json.convertValue(result, Object.class);
		} catch (Exception fetchFailure) {
			log.warn("template data fetch failed for '{}'", path, fetchFailure);
			return null;
		} finally {
			inFetch.remove();
		}
	...
	public boolean canResolve(String source) {
		if (source == null || !source.startsWith("/"))
			return false;
		return resolve(pathPortion(source), new HashMap<>()).isPresent();
	}
	...
	private Optional<Endpoint> resolve(String path, Map<String, String> params) {
	...
	private void register(Map<String, Endpoint> exact, Map<String, Endpoint> trailing, String pattern,
		...
		require(braceIndex > 0 && pattern.endsWith("}") && pattern.charAt(braceIndex - 1) == '/'
				&& pattern.indexOf('{', braceIndex + 1) < 0, pattern, "only a single trailing {variable} is supported");
		String prefix = pattern.substring(0, braceIndex - 1);
		String variable = pattern.substring(braceIndex + 1, pattern.length() - 1);
	...
	private Object invoke(Endpoint endpoint, Map<String, String> params) {
		...
		} catch (ReflectiveOperationException invocationFailure) {
			throw invocationFailure.getCause() instanceof RuntimeException cause ? cause
					: new IllegalStateException(invocationFailure);
		}
	}
