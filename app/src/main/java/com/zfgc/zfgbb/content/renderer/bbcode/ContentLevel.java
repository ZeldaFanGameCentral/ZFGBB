...
public enum ContentLevel {
	...
	public static ContentLevel contentLevelOfMarkup(String openMarkup, String closeMarkup) {
		return anyElementNamedIsBlockLevel(openMarkup) || anyElementNamedIsBlockLevel(closeMarkup)
				? BLOCK
				: INLINE;
	}

	public static Set<ContentLevel> contentLevelsDeclaredBy(BBCodeConfig config) {
		Set<ContentLevel> declared = new LinkedHashSet<>();
		for (BBCodeAttributeMode mode : config.getAttributeConfig().values())
			declared.add(contentLevelOfMarkup(mode.getOpenTag(), mode.getCloseTag()));
		if (declared.isEmpty())
			declared.add(contentLevelOfMarkup("", config.getEndTag()));
		return declared;
	}

	public static List<String> elementNamesIn(String markup) {
	...
	private static boolean anyElementNamedIsBlockLevel(String markup) {
		for (String elementName : elementNamesIn(markup))
