...
public final class BBCodeParser {
	...
	public record ExpandedOpener(BBCodeConfig config, String openMarkup, OptionalInt contentSlotIndex,
		...
		public boolean itNamesASourceToPullFrom() {
			return config.referencesSourceContent() && attributeMode
					.flatMap(mode -> parsedAttributes.theValueOfTheAttributeDeclaredBy(mode,
		...
		private String itsMarkupWithTheContentSlotHolding(String bodySource) {
	...
	private Optional<OpenTag> theTagAwaitingItsContentSlot = Optional.empty();
	...
	BBCodeDocument parseDocument() {
		while (cursor < length) {
			if (theImplicitItemMarkerOpensHere()) {
	...
	public static Optional<TagToken> theTagWrittenAt(String source, int bracketIndex) {
		return new BBCodeParser(source, Map.of()).theTagWrittenAt(bracketIndex);
	}

	Optional<TagToken> theTagWrittenAt(int bracketIndex) {
	...
	public static Optional<ExpandedOpener> theTagAnOpeningCodeExpandsTo(BBCodeConfig config, String attributeText) {
		...
		if (attributeMode == null)
			return Optional.empty();
		String openMarkup =
				theMarkupWithEveryAttributeSlotFilled(attributeMode.getOpenTag(), attributeMode, parsed);
		...
		return Optional.of(new ExpandedOpener(config, openMarkup,
				...
				ContentLevel.theContentLevelOfMarkup(openMarkup, closeMarkup), closeMarkup, parsed,
				Optional.of(attributeMode)));
	}

	public static String theMarkupWithEveryAttributeSlotFilled(String markup, BBCodeAttributeMode attributeMode,
		...
		for (int index = 0; index < attributeMode.getAttributes().size(); index++) {
			...
			String value = attribute.getDataType() == AttributeDataType.TIMESTAMP
					? BBCodeDateElement.theEpochSecondsAsADateElement(rawValues.get(index))
	...
	private boolean isTagCodeCharacter(char candidate) {
	...
	private int consumeClosingTag(RecognisedTag tag) {
		...
		if (!anyTagHasOpened)
			return tag.endIndex();
		if (theOpenImplicitItemBelongsTo(tag.code()))
	...
	private int consumeSelfClosingTag(RecognisedTag tag, BBCodeConfig config) {
		...
		currentChildren().add(new BBCodeTag(config, config.getEndTag(), Optional.empty(),
				ContentLevel.theContentLevelOfMarkup(config.getEndTag(), config.getEndTag()),
	...
	private int consumeOpeningTag(RecognisedTag tag, BBCodeConfig config) {
		...
		if (expanded.get().contentSlotIndex().isPresent())
			theTagAwaitingItsContentSlot = Optional.of(opened);
		anyTagHasOpened = true;
		return tag.endIndex();
	}

	private void fillTheContentSlotFrom(int closingBracketIndex) {
		if (theTagAwaitingItsContentSlot.isEmpty())
			return;
		theTagAwaitingItsContentSlot.get()
				.fillItsContentSlotFrom(source.substring(pendingTextFrom, closingBracketIndex));
		theTagAwaitingItsContentSlot = Optional.empty();
	}

	private OpenTag pushOpenTag(ExpandedOpener opener, String code, AuthoredSource authoredSource,
			Optional<OpenTag> itsImplicitContainer) {
		int openInstanceOrdinal = openInstanceCountsByCode.getOrDefault(code, 0);
		OpenTag opened = new OpenTag(opener, code, authoredSource, openInstanceOrdinal, itsImplicitContainer);
		openTags.push(opened);
		openInstanceCountsByCode.put(code, openInstanceOrdinal + 1);
		return opened;
	}

	private boolean theImplicitItemMarkerOpensHere() {
		return theContainerAwaitingImplicitItems()
				.filter(container -> source.startsWith(container.itsImplicitItemMarker().orElseThrow(), cursor))
				.isPresent();
	}

	private Optional<OpenTag> theContainerAwaitingImplicitItems() {
		OpenTag innermost = openTags.peek();
		if (innermost == null)
			return Optional.empty();
		OpenTag container = innermost.itsImplicitContainer().orElse(innermost);
		return container.itsImplicitItemMarker().isPresent()
				&& grammar.containsKey(container.itsImplicitItemCode())
						? Optional.of(container)
						: Optional.empty();
	}

	private void openTheNextImplicitItem() {
		OpenTag container = theContainerAwaitingImplicitItems().orElseThrow();
		int markerEnd = cursor + container.itsImplicitItemMarker().orElseThrow().length();
		...
		BBCodeConfig itemConfig = grammar.get(container.itsImplicitItemCode());
		...
		expanded.ifPresent(opener -> pushOpenTag(opener, container.itsImplicitItemCode(),
				new AuthoredSource("", cursor, length, markerEnd - cursor, 0), Optional.of(container)));
		cursor = markerEnd;
	}

	private boolean theOpenImplicitItemBelongsTo(String containerCode) {
		OpenTag innermost = openTags.peek();
		return innermost != null && innermost.itsImplicitContainer()
	...
	private final class OpenTag {
		...
		private final Optional<OpenTag> itsImplicitContainer;
		...
		private boolean awaitingItsAuthorsCloser = true;
		...
		private OpenTag(ExpandedOpener opener, String code, AuthoredSource authoredSource, int openInstanceOrdinal,
			...
			this.itsImplicitContainer = itsImplicitContainer;
		}
		...
		private Optional<OpenTag> itsImplicitContainer() {
			return itsImplicitContainer;
		}

		private boolean openedByAnImplicitMarker() {
			return itsImplicitContainer.isPresent();
		}

		private Optional<String> itsImplicitItemMarker() {
			return opener.config().declaredImplicitItemMarker();
		}

		private String itsImplicitItemCode() {
		...
		private void recordThatItsAuthorsCloserArrived() {
			awaitingItsAuthorsCloser = false;
		}
		...
		private void fillItsContentSlotFrom(String bodySource) {
			openMarkup = opener.itsMarkupWithTheContentSlotHolding(bodySource);
		}

		private BBCodeTag finished() {
			return itsNode(Optional.ofNullable(opener.closeMarkup()));
		}

		private BBCodeTag finishedWithoutACloser() {
			return itsNode(Optional.empty());
		}

		private BBCodeTag itsNode(Optional<String> closeMarkup) {
			...
			return new BBCodeTag(opener.config(), openMarkup, closeMarkup, opener.contentLevel(),
					...
					awaitingItsAuthorsCloser && !openedByAnImplicitMarker(), openedByAnImplicitMarker(), children);
		}
	}

	private void trimTheBlankEdgesTheContainerDiscards(List<BBCodeNode> children) {
		boolean trimming = true;
		while (trimming && !children.isEmpty())
			trimming = theBlankEdgeTrimmedOffTheFirstChild(children);
		trimming = true;
		while (trimming && !children.isEmpty())
			trimming = theBlankEdgeTrimmedOffTheLastChild(children);
	}

	private boolean theBlankEdgeTrimmedOffTheFirstChild(List<BBCodeNode> children) {
		if (!(children.get(0) instanceof BBCodeText text))
			return false;
		String kept = BBCodeText.withoutItsLeadingBlankRun(text.sourceText());
	...
	private boolean theBlankEdgeTrimmedOffTheLastChild(List<BBCodeNode> children) {
		int last = children.size() - 1;
		if (!(children.get(last) instanceof BBCodeText text))
			return false;
		String kept = BBCodeText.withoutItsTrailingBlankRun(text.sourceText());
		if (kept.equals(text.sourceText()))
			return false;
		if (kept.isEmpty())
			children.remove(last);
		else
			children.set(last, new BBCodeText(kept, text.escaping()));
		return kept.isEmpty();
	}
