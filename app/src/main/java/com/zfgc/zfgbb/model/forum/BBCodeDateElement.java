...
public record BBCodeDateElement(String isoTimestamp) {
	...
	public static String epochSecondsAsDateElement(String value) {
		String epochSeconds = value == null ? "" : value.trim();
		if (epochSeconds.isEmpty() || !epochSeconds.chars().allMatch(digit -> digit >= '0' && digit <= '9'))
			return "";
		try {
			return of(OffsetDateTime.ofInstant(Instant.ofEpochSecond(Long.parseLong(epochSeconds)), ZoneOffset.UTC))
					.toHtml();
		} catch (NumberFormatException | DateTimeException outOfRange) {
			return "";
		}
	}
	...
	public Optional<String> longFormText() {
		try {
