...
public interface QuoteStripConversionMapper {
	...
	class QuoteStripRunSummary {
		...
		public void setCandidateRows(int candidateRows) {
			this.candidateRows = candidateRows;
		}

		public int getPlannedRows() {
			return plannedRows;
		}

		public void setPlannedRows(int plannedRows) {
	...
	@Select("select message_id from zfgbb.message_history where current_flag = true "
			+ "group by message_id having count(*) > 1")
	List<Integer> loadDuplicateCurrentMessageIds();

	@Select("select message_text as messageText, created_ts as createdTs from zfgbb.message_history "
			+ "where current_flag = true and message_text like '%[quote%'")
