...
public interface TestQueryHelperMapper {
	...
	@Select("select code from zfgbb.content_format order by code")
	List<String> listContentFormatCodes();

	@Select("""
			select count(*) from zfgbb.reaction given
			where given.reactable_type = 'MESSAGE' and not exists
				(select 1 from zfgbb.message post where post.message_id = given.reactable_id)
			""")
	long countReactionsWithoutMessage();

	@Select("""
			select count(*) from zfgbb.reaction given
			where given.reactable_type = 'WIKI_PAGE' and not exists
				(select 1 from zfgbb.wiki_page page where page.wiki_page_id = given.reactable_id)
			""")
	long countReactionsWithoutWikiPage();

	@Select("""
			select count(*) from zfgbb.reaction given
			where given.reactable_type in ('PROJECT', 'RESOURCE') and not exists
				(select 1 from zfgbb.content_entity entity
				  where entity.content_entity_id = given.reactable_id
				    and entity.entity_type = given.reactable_type)
			""")
	long countReactionsWithoutContentEntity();
	...
	@Select("""
			select zfgbb.seed_content_template(#{code}, #{contentFormat}, #{scope}, #{source}, #{body})
			""")
	String seedContentTemplate(@Param("code") String code, @Param("contentFormat") String contentFormat,
			@Param("scope") String scope, @Param("source") String source, @Param("body") String body);
